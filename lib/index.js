...
const runHotato = Promise.coroutune(function * (resolvedModules, loopFn) {
  ...
  while (true) {
    ...
    if (!resolved) {
      ...
      console.log(`${prefix} Look up OK, with returned value: ${util.inspect(returnValue)}`)
    }
...
function awaitInput () {
  return new Promise((resolve, reject) => {
    console.log(`${prefix} Awaiting input. Press "c" to continue, or "r" to re-run loop.`)
